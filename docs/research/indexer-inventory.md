...
2. **Protocols**: op-drop (`$DROP` fungible tokens + trading desk) and Drops (inscription/artifact indexer + `/drops` HTML gallery) over one confirmed-chain scan.
...
2. **Protocols**: Drops (base-layer Taproot OP_DROP artifact protocol, BIP341-verified; recognizes registered OP_DROP carrier; reference-only Pacts profile). 256-byte body cap.
...
2. **Protocols**: op-drop ($DROP) + Drops over one scan, the direct predecessor of index-drops-and-opdrop.
