...
export async function verifyOwnedTransactionInput(raw: unknown, inputIndex: unknown, core: WorkbenchCoreReader = ownedWorkbenchCore) {
  if (active >= 2) throw new CompilerError('verification-busy', 'Owned transaction verification capacity is occupied.', 503);
  active++;
  const deadline = Date.now() + 15000;
  const pending = verifyInput(raw, inputIndex, core, deadline).finally(() => { active--; });
  let timer: NodeJS.Timeout | undefined;
  try {
    return await Promise.race([pending, new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => reject(new CompilerError('verification-timeout', 'Owned transaction verification exceeded its deadline.', 503)), 15000);
    })]);
...
async function verifyInput(raw: unknown, inputIndex: unknown, core: WorkbenchCoreReader, deadline: number) {
  ...
  const checkDeadline = () => { if (Date.now() >= deadline) throw new CompilerError('verification-timeout', 'Owned transaction verification exceeded its deadline.', 503); };
  const call = async (method: string, params: unknown[]) => { checkDeadline(); try { return await core.call(method,params); } catch { throw new CompilerError('missing-transaction-context', 'The owned source cannot resolve the required transaction context.', 503); } };
  const genesis = await call('getblockhash',[0]);
  if (!GENESIS[core.network] || genesis !== GENESIS[core.network]) throw new CompilerError('wrong-network','Owned source genesis does not match the selected network.',503);
  const before = await call('getblockchaininfo',[]);
  if (!expected || before.chain !== expected || !Number.isSafeInteger(before.blocks) || before.blocks < 0 || !/^[0-9a-f]{64}$/.test(before.bestblockhash)) throw new CompilerError('wrong-network','Owned source network/checkpoint mismatch.',503);
  ...
  if (after.chain!==before.chain || after.bestblockhash!==before.bestblockhash || after.blocks!==before.blocks) throw new CompilerError('source-changed','Chain checkpoint changed; retry verification.',503);
