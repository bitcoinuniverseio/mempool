 ...
 * address index fails the release gate, so every cutover to an Electrum
 * deployment rolled back while the index was healthy and answering.
 *
 * Takes the request function rather than a client so the wire call it
 * makes can be asserted without a server. The arity is the whole bug, and
 * a test that cannot see the arity cannot catch it.
 */
export async function readIndexedTip(
  ...
  try {
    ...
    // Warn, not debug. This is the only thing that decides whether the
    // address index looks reachable, and a release gate refuses to ship
    // when it does not. Hiding that decision at debug level is how a
    // healthy index came to look unreachable with nothing in the log.
    logger.warn('Electrum did not report its indexed height: ' + (e instanceof Error ? e.message : e));
    return null;
  }
}
