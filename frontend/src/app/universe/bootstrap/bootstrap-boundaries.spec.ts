...
describe('bootstrap source and request boundaries',()=>{
 ...
 it('does not promote truthy validity to safe-to-load or convert503 into mismatch',()=>{
   const pending=new Subject<any>();const c=new BootstrapVerifyComponent({verifySnapshotChecksum$:()=>pending} as any,{markForCheck:vi.fn()} as any);
   c.verifyChecksum();pending.next({valid:'true',status:'pinned_core_verified'});expect(c.report).toBeNull();expect(c.failure).toContain('Supplied hashes');
   pending.error({status:503,error:{error:'Trusted snapshot source unavailable'}});expect(c.report).toBeNull();expect(c.failure).toContain('unavailable');
 });
