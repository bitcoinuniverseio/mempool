    ...
    <div class="intelligence-page container-xl">
      <header class="page-header mb-4">
        ...
        <p class="subtitle text-muted mt-2 mb-3">
          Live monitoring of Bitcoin Core nodes running dual chainstates:
          snapshot chainstate (tip sync) vs background chainstate (historical
          IBD).
        </p>
        ...
        >
          <a class="nav-link" [routerLink]="'/node/bootstrap' | relativeUrl"
            >Overview</a
          >
          <a
            class="nav-link"
          ...
          <div class="card p-4 h-100 bg-body-tertiary border">
            ...
            >
              ...
              <span
                class="badge"
                [ngClass]="
                  n.current_phase === 'fully_validated'
                    ? 'bg-success'
                    : 'bg-info text-dark'
                "
              >
            ...
            <p class="small text-muted">
              Observed: {{ n.observed_at | date: 'medium' }}
            </p>
            <div class="my-3">
              <div class="d-flex justify-content-between small text-muted mb-1">
                <span>Core Estimated Verification Progress</span>
                <span class="fw-bold font-monospace"
                  >{{ n.sync_percent.toFixed(1) }}%</span
                >
              </div>
              <div class="progress" style="height: 10px;">
                <div
                  class="progress-bar progress-bar-striped progress-bar-animated"
                  [style.width.%]="n.sync_percent"
                ></div>
              </div>
            </div>
            ...
            <div class="row g-2 mb-3">
              <div class="col-6">
                <div class="p-2 border rounded bg-body">
                  ...
                  <div class="fw-bold font-monospace">
                    {{
                      n.background_ibd_height === null
                        ? 'Not active'
                        : '#' + n.background_ibd_height
                    }}
                  </div>
                </div>
              </div>
              <div class="col-6">
                <div class="p-2 border rounded bg-body">
                  <div class="text-muted small">Active Chainstate Tip</div>
            ...
            <div class="p-2 border rounded bg-body mb-3">
              <div class="text-muted small">
                Estimated Background Validation Completion
              </div>
              <div class="small font-monospace fw-bold">Not measured</div>
            </div>

            <div class="alert alert-info py-2 px-3 small m-0 mt-auto">
              Once background IBD reaches the snapshot block height, Bitcoin
              Core confirms the state hashes match and gracefully decommissions
              the background chainstate.
            </div>
          </div>
        </div>
      </div>
      ...
      .pipe(
        switchMap(() => {
          ...
          return timer(0, 15000).pipe(
            switchMap(() =>
              this.bootstrapApi.getNodeChainstates$().pipe(
                catchError((err) => {
                  this.nodes = [];
                  this.error =
                    err?.error?.error ||
                    'Owned chainstate evidence unavailable';
                  return of(null);
                })
              )
            )
