      ...
      <header class="page-header mb-4">
        ...
        >
          ...
          <span
            ...
            *ngIf="overview && overview.recommended_snapshot_height !== null"
          >
            Recommended Snapshot: #{{ overview.recommended_snapshot_height }}
          </span>
        </div>
        <p class="subtitle text-muted mt-2 mb-3">
          Authoritative AssumeUTXO snapshot directory, UTXO set hash
          commitments, dual-chainstate sync telemetry, and fast node bootstrap
          planner.
        </p>
        ...
        >
          <a
            ...
            [routerLink]="'/node/bootstrap' | relativeUrl"
            >Overview</a
          >
          <a
            class="nav-link"
          ...
          <a
            class="nav-link"
            [routerLink]="'/node/bootstrap/verify' | relativeUrl"
            >Integrity Verifier</a
          >
          <a
            class="nav-link"
            [routerLink]="'/node/bootstrap/planner' | relativeUrl"
            >Bootstrap Planner</a
          >
          <a
            class="nav-link"
            [routerLink]="'/node/bootstrap/chainstates' | relativeUrl"
            >Dual Chainstates</a
          >
        </nav>
      </header>
      ...
      <div *ngIf="!loading && overview" class="row g-4">
        ...
        <div class="col-12 col-md-3">
          <div class="card p-3 bg-body-tertiary border h-100">
            ...
            <div class="fs-4 fw-bold mt-1">
              {{ overview.configured_nodes_count }}
            </div>
        ...
        <div class="col-12 col-md-3">
          <div class="card p-3 bg-body-tertiary border h-100">
            ...
            <div class="fs-4 fw-bold text-success mt-1">
              {{ overview.dual_chainstate_nodes_count }}
            </div>
        ...
        <div class="col-12 col-md-3">
          <div class="card p-3 bg-body-tertiary border h-100">
            ...
            <div class="fs-4 fw-bold text-primary mt-1">Not measured</div>
            <div class="small text-muted mt-1">
              Requires a measured bootstrap run
            </div>
          </div>
        </div>
        ...
        <div class="col-12 col-lg-8">
          <div class="card p-4 bg-body-tertiary border h-100">
            ...
            <div
              ...
              tabindex="0"
              role="region"
              aria-label="Pinned AssumeUTXO Snapshots, scroll horizontally"
              i18n-aria-label
            >
                ...
                <tbody>
                  <tr *ngFor="let s of overview.featured_snapshots">
                    <td>
                      <a
                        [routerLink]="[
                          '/node/bootstrap/snapshot' | relativeUrl,
                          s.height,
                        ]"
                        class="fw-bold font-monospace text-decoration-none"
                      >
                        #{{ s.height }}
                      </a>
                    </td>
        ...
        <div class="col-12 col-lg-4">
          <div class="card p-4 bg-body-tertiary border h-100">
            ...
            <p class="small text-muted mb-3">
              AssumeUTXO loads an authentic serialized UTXO set at a hard-coded
              checkpoint block. The node can immediately participate in
              consensus and serve requests from the snapshot height while
              simultaneously running full validation of historical blocks from
              the genesis block in the background.
            </p>
            <a
              [routerLink]="'/node/bootstrap/planner' | relativeUrl"
...
export class BootstrapOverviewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.sub = this.bootstrapApi.networkChanged$
      .pipe(
        switchMap(() => {
          this.overview = null;
          this.loading = true;
          ...
          return timer(0, 15000).pipe(
            switchMap(() =>
              this.bootstrapApi.getOverview$().pipe(
                catchError((err) => {
                  this.overview = null;
                  this.error =
                    err?.error?.error || 'Owned bootstrap evidence unavailable';
                  return of(null);
                })
              )
            )
