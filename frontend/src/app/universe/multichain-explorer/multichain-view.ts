...
export function readStatusRail(
  ...
  return [
    ...
    { id: 'lag', label: 'Node behind network', value: lag ? lag.display + ' blocks' + (current ? '' : ' (last known)') : 'Not stated', tone: current && lag?.exact === '0' ? 'proven' : 'neutral', exact: lag?.exact ?? null },
...
export function readProtocolCoverage(
  ...
  const health = readHealth(capability);
  const declared = health ? health.protocols.map(row => ({
    ...row,
