...
export interface AcceleratorDirectoryRef {
  source: string;
  revision: string;
  loaded_at_utc: string;
}

/** A provider from the owned directory file, which is the trust root. */
export interface AcceleratorProvider {
  ...
  /** Null: entries carry no separate signature; the directory file is the trust root. */
  provider_signature: string | null;
  issuer?: string;
  protocol_version?: string;
  keys?: AcceleratorProviderKey[];
  directory?: AcceleratorDirectoryRef;
}
...
/**
 * invalid: structure, completeness or signature failed; unsupported: an
 * encoding this deployment cannot verify; unavailable-trust: no directory, or
 * the provider or key is not in it or not valid at issue; wrong-network;
 * expired; duplicate: a replay of a receipt already verified here; verified.
 * HTTP: 200 verified, 409 duplicate, 503 unavailable-trust, 400 otherwise.
 */
...
export const RECEIPT_VERIFICATION_STAGES: readonly ReceiptVerificationStage[] = [
  'invalid',
  'unsupported',
  'unavailable-trust',
  'wrong-network',
...
/** The findings envelope; no-blocks-observed means this observer has recorded no block yet. */
export interface OrderingFindings {
  network: string;
  state: 'observed' | 'no-blocks-observed';
  findings: TransactionOrderingEvidence[];
  coverage: Array<{ block_hash: string; height: number; template_coverage: 'template-observed' | 'no-template-observed'; transactions: number; findings: number }>;
...
/** What GET submission/overview answers: the owned relay, its queue and its worker. */
...
export type SubmissionOverview = PrivateRelayOverview;
