...
@Component({
  ...
  template: `
    <div class="holdings">
      <header class="toolbar">
        <label class="search">
          <span
            ...
            i18n="@@universe.portfolio.holdings.search-label"
            >Search holdings</span
          >
          <input
            type="search"
            [value]="query()"
        ...
        <label class="group">
          ...
          <select #groupSelect (change)="group.set($any(groupSelect.value))">
            <option
              value="asset"
              i18n="@@universe.portfolio.holdings.group-asset"
            >
              Asset
            </option>
            <option
              value="account"
              i18n="@@universe.portfolio.holdings.group-account"
            >
              Account
            </option>
            <option
              value="chain"
              i18n="@@universe.portfolio.holdings.group-chain"
            >
              Chain
            </option>
            <option
              value="protocol"
              i18n="@@universe.portfolio.holdings.group-protocol"
            >
              Protocol
            </option>
            <option
              value="priced"
              i18n="@@universe.portfolio.holdings.group-priced"
            >
              Priced / unpriced
            </option>
          </select>
      ...
      @if (data().loading && rows().length === 0) {
        <p role="status" i18n="@@universe.portfolio.holdings.loading">
          Loading holdings…
        </p>
      } @else if (rows().length === 0) {
        <p class="soft" i18n="@@universe.portfolio.holdings.empty">
          No holdings yet. Add accounts, or refresh once the addresses have
          activity.
        </p>
      } @else {
        <!-- Desktop table -->
        <div class="table-wrap">
          <table>
            <caption
              class="visually-hidden"
              i18n="@@universe.portfolio.holdings.caption"
            >
              Holdings with exact quantities, values, and source states
            </caption>
            <thead>
              <tr>
                <th scope="col" i18n="@@universe.portfolio.holdings.col-asset">
                  Asset
                </th>
                <th
                  scope="col"
                  class="num"
                  i18n="@@universe.portfolio.holdings.col-quantity"
                >
                  Exact quantity
                </th>
                <th
                  scope="col"
                  class="num"
                  i18n="@@universe.portfolio.holdings.col-value"
                >
                  Priced value
                </th>
                <th
                  scope="col"
                  class="num"
                  i18n="@@universe.portfolio.holdings.col-share"
                >
                  Share
                </th>
                <th scope="col" i18n="@@universe.portfolio.holdings.col-state">
                  State
                </th>
              </tr>
            </thead>
                ...
                <tr>
                  <td>
                    <button
                      ...
                      (click)="
                        expanded.set(
                          expanded() === row.holding.assetKey
                            ? ''
                            : row.holding.assetKey
                        )
                      "
                      [attr.aria-expanded]="expanded() === row.holding.assetKey"
                    >
                      <strong>{{
                        row.holding.displayName ??
                          row.holding.ticker ??
                          shortKey(row.holding)
                      }}</strong>
                  ...
                  <td class="num">
                    {{
                      session.valuesHidden() ? masked() : quantity(row.holding)
                    }}
                  </td>
                  <td class="num">
                    {{
                      session.valuesHidden()
                        ? masked()
                        : row.holding.pricedValue === null
                          ? '-'
                          : formatExact(row.holding.pricedValue, 'en')
                    }}
                  </td>
                    ...
                    <td colspan="5">
                      <div class="expansion">
                        <p
                          ...
                          i18n="@@universe.portfolio.holdings.locations"
                        >
                          Locations
                        </p>
                        <ul>
                          @for (
                            location of row.holding.locations;
                            track location.reference
                          ) {
                            <li class="mono">
                              <span>{{
                                location.kind === 'outpoint'
                                  ? 'Output'
                                  : location.kind === 'protocol-ledger'
                                    ? 'Protocol ledger'
                                    : 'Manual'
                              }}</span>
                              <span>{{ location.reference }}</span>
                              <span>{{
                                location.quantityAtomic === null
                                  ? '-'
                                  : quantityText(
                                      location.quantityAtomic,
                                      row.holding.decimals
                                    )
                              }}</span>
                            </li>
                          }
                        ...
                        <p
                          class="soft"
                          i18n="@@universe.portfolio.holdings.deep-links"
                        >
                          Asset details live on their protocol protocol pages -
                          open them from the activity timeline.
                        </p>
                      </div>
                    </td>
                  </tr>
  ...
  styles: [
    `
      ...
      .expander:hover {
        background: var(--u-surface-raised, rgba(0, 0, 0, 0.03));
      }
      .sub {
        display: block;
        font-size: 11.5px;
...
export class HoldingsComponent {
  ...
  private shareOf(holding: AggregatedHolding, total: string | null): string {
    if (holding.pricedValue === null || total === null || total === '0')
      return '-';
