    ...
    <div class="manage">
      <header class="head">
        ...
        <a
          ...
          i18n="@@universe.portfolio.manage.new"
          >New portfolio</a
        >
      </header>
      ...
      @if (store.vaultKind() !== 'unlocked') {
        <p class="soft" i18n="@@universe.portfolio.manage.locked">
          The vault is locked. Unlock from the portfolio home to manage stored
          portfolios.
        </p>
      }
          ...
          <li>
              ...
              <div class="info">
                @if (renaming() === portfolio.id) {
                  <input
                    ...
                    (keydown.enter)="
                      confirmRename(portfolio.id, renameInput.value)
                    "
                  />
                  <button
                    type="button"
                    (click)="confirmRename(portfolio.id, renameInput.value)"
                    i18n="@@universe.portfolio.manage.save"
                  >
                    Save
                  </button>
                } @else {
                  ...
                  <span class="meta">
                    ...
                    @if (portfolio.archived) {
                      ·
                      <span i18n="@@universe.portfolio.manage.archived"
                        >archived</span
                      >
                    }
                  </span>
              ...
              <div class="actions">
                @if (!portfolio.archived) {
                  <a
                    ...
                    i18n="@@universe.portfolio.manage.open"
                    >Open</a
                  >
                }
                <button
                  type="button"
                  (click)="renaming.set(portfolio.id)"
                  i18n="@@universe.portfolio.manage.rename"
                >
                  Rename
                </button>
                <button
                  type="button"
                  (click)="duplicate(portfolio)"
                  i18n="@@universe.portfolio.manage.duplicate"
                >
                  Duplicate
                </button>
                ...
                <button
                  ...
                  i18n="@@universe.portfolio.manage.delete"
                >
                  Delete…
                </button>
              </div>
            ...
            @if (findDuplicateAddresses(portfolio).length > 0) {
              <p
                class="warning"
                role="note"
                i18n="@@universe.portfolio.manage.duplicates"
              >
                Some addresses appear under more than one account - aggregation
                counts them once.
              </p>
            }
          </li>
      ...
      @if (deleting(); as id) {
        <div class="dialog" role="alertdialog" aria-labelledby="delete-title">
          <div class="dialog-panel">
            <h2
              id="delete-title"
              i18n="@@universe.portfolio.manage.delete-title"
            >
              Delete this portfolio?
            </h2>
            <p i18n="@@universe.portfolio.manage.delete-copy">
              This removes the local portfolio definition, its accounts, labels,
              views, and snapshots from the encrypted vault on this device. Your
              blockchain assets are untouched - this deletes only what this
              browser stored.
            </p>
            <div class="actions">
              <button
                ...
                i18n="@@universe.portfolio.manage.delete-confirm"
              >
                Delete local data
              </button>
              <button
                type="button"
                (click)="deleting.set('')"
                i18n="@@universe.portfolio.manage.cancel"
              >
                Cancel
              </button>
            </div>
...
export class ManagePortfoliosComponent {
  ...
  protected async confirmRename(id: string, name: string): Promise<void> {
    ...
    await this.store.updatePortfolio(id, (portfolio) => ({
      ...portfolio,
      name: trimmed,
    }));
    this.renaming.set('');
  ...
  protected async duplicate(portfolio: {
    id: string;
    name: string;
  }): Promise<void> {
    const source = this.store
      .portfolios()
      .find((candidate) => candidate.id === portfolio.id);
  ...
  protected async toggleArchive(portfolio: {
    id: string;
    archived: boolean;
  }): Promise<void> {
