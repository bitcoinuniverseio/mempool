/**
 * Local secret-input defenses for Portfolio Intelligence.
 *
 * Before any network request and before anything is stored, pasted
 * material is classified. Anything that looks like a private credential -
 * an extended private key, a WIF key, a likely mnemonic, raw private-key
 * hex in a private-key context, a seed export, or an unsupported wallet
 * backup - is rejected locally with a safety message.
 *
 * The detection result never echoes the input: the caller clears the
 * input model and shows the reason only. `looksSecretLike` inspects
 * structure, not dictionaries, and keeps no state.
 */
...
const EXTENDED_KEY_PREFIXES = /^(?:x|y|z|X|Y|Z|t|u|v|T|U|V)(?:prv|priv)/;
const BASE58_EXTENDED_PRV = /^(?:[1-9A-HJ-NP-Za-km-z]{52,111})$/;
const WIF = /^[5KL][1-9A-HJ-NP-Za-km-z]{50,51}$/;
const HEX_64 = /^[\da-fA-F]{64}$/;
const SEED_WORDS = [
  'abandon', 'ability', 'able', 'about', 'above', 'absent', 'absorb', 'abstract',
  'absurd', 'abuse', 'access', 'accident', 'account', 'accuse', 'achieve', 'acid',
  'across', 'act', 'action', 'actor', 'actress', 'actual', 'adapt', 'addict',
  ...
  'approve', 'april', 'arcade', 'arch', 'arctic', 'area', 'arena', 'argue', 'arm',
  ...
  'avocado', 'awake', 'aware', 'away', 'awesome', 'awful', 'awkward', 'axis',
  ...
  'fire', 'firm', 'first', 'fiscal', 'fish', 'fit', 'fitness', 'fix', 'flag',
  ...
  'found', 'fox', 'fragile', 'frame', 'frequent', 'fresh', 'friend',
  ...
  'know', 'lab', 'label', 'labor', 'ladder', 'lady', 'lagoon', 'lake',
  'lamb', 'lamp', 'language', 'lantern', 'lap', 'large', 'later', 'latin',
  ...
  'merger', 'merry', 'mesh', 'message', 'metal', 'method', 'middle', 'midnight',
  ...
  'sudden', 'suffer', 'sugar', 'suggest', 'suit', 'summer', 'sun',
  'sunny', 'sunset', 'super', 'supply', 'supreme', 'sure', 'surface',
  ...
  'truck', 'true', 'truly', 'Trump', 'trust', 'truth', 'try', 'tube',
  ...
  'use', 'used', 'useful', 'useless', 'usual', 'utility', 'vacant',
...
const WORD_SPLIT = /[\s\u00a0]+/;
const MNEMONIC_MIN_WORDS = 11; // 12-word phrases with one stray split still trip

const BACKUP_SUFFIXES = /\.(?:seed|wallet|keystore|bak|backup)$/i;

/** Structural detection: no dictionaries of addresses, no network calls. */
export function looksSecretLike(input: string): SecretDetection {
  const text = (input ?? '').trim();
  if (text.length === 0) return { secret: false, kind: null };

  if (EXTENDED_KEY_PREFIXES.test(text)) {
    return { secret: true, kind: 'extended-private-key' };
  }
  if (WIF.test(text)) {
    return { secret: true, kind: 'wif-private-key' };
  }
  if (HEX_64.test(text) && !/^(?:0{2}[0-9a-f]{2})+$/i.test(text)) {
    // Raw 32-byte hex is the standard private-key shape; anything that
    // decodes as a txid arrives through a different field.
    return { secret: true, kind: 'raw-private-key-hex' };
  }
  if (BACKUP_SUFFIXES.test(text)) {
    return { secret: true, kind: 'seed-export-file' };
  }
  const words = text.toLowerCase().split(WORD_SPLIT).filter((word) => word.length > 0);
  if (words.length >= MNEMONIC_MIN_WORDS) {
    const known = words.filter((word) => SEED_WORDS.includes(word)).length;
    if (known / words.length >= 0.9) {
      return { secret: true, kind: 'mnemonic-phrase' };
    }
  }
  // Base58 strings of extended-private-key length that no public purpose
  // explains: xpubs are 111 chars and start with xpub/ypub/zpub (public
  // prefixes are checked first by the caller), WIF is handled above, so a
  // bare 52-char base58 blob in a watch-only field is treated as a key.
  if (BASE58_EXTENDED_PRV.test(text) && text.length < 52) {
    return { secret: true, kind: 'wif-private-key' };
  }
  return { secret: false, kind: null };
}

/** The safety copy for a detected kind. Never echoes the input. */
export function secretRejectionCopy(kind: SecretKind): string {
  switch (kind) {
    case 'extended-private-key':
      return 'That looks like an extended PRIVATE key. Universe is watch-only: private keys never belong here, and this input was discarded without being stored or sent.';
    case 'wif-private-key':
      return 'That looks like a private key in WIF form. Universe is watch-only: this input was discarded without being stored or sent.';
    case 'mnemonic-phrase':
      return 'That looks like a recovery phrase (seed words). Never paste a recovery phrase anywhere except your own wallet restore. This input was discarded without being stored or sent.';
    case 'raw-private-key-hex':
      return 'That looks like a raw private key. Universe is watch-only: this input was discarded without being stored or sent.';
    case 'seed-export-file':
      return 'That looks like a seed-export or wallet file name. Universe cannot import wallet backups: this input was discarded.';
    case 'wallet-backup-file':
      return 'That looks like a wallet backup file. Universe cannot import wallet backups: this input was discarded.';
  }
}

/** True when the string is a public extended key we can derive from. */
export function looksLikePublicExtendedKey(input: string): boolean {
  return /^(?:xpub|ypub|zpub|tpub|upub|vpub)[1-9A-HJ-NP-Za-km-z]{40,}$/.test(
    (input ?? '').trim(),
  );
}

/** True when the string is a plausibly checksummed output descriptor. */
export function looksLikeDescriptor(input: string): boolean {
  return /^(?:pkh|wpkh|sh\(wpkh|tr|addr)\(?[^\s]+$/.test((input ?? '').trim());
}
