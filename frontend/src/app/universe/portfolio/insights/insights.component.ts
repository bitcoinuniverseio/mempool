    ...
    <div class="insights">
      @if (insights().length === 0) {
        <p class="soft" i18n="@@universe.portfolio.insights.empty">
          Nothing needs attention. Insights appear when the measured state of
          the portfolio crosses a stated, explainable rule - never from a score.
        </p>
      }
      <ul>
        @for (insight of insights(); track insight.insightId) {
          <li [attr.data-severity]="insight.severity">
            ...
            <p
              class="confidence"
              i18n="@@universe.portfolio.insights.confidence"
            >
            ...
            @if (dismissed().includes(insight.insightId)) {
              <button
                type="button"
                (click)="restore(insight)"
                i18n="@@universe.portfolio.insights.restore"
              >
                Restore
              </button>
            } @else {
              <button
                ...
                i18n="@@universe.portfolio.insights.dismiss"
              >
                Dismiss
              </button>
            }
...
export class InsightsComponent {
  ...
  protected restore(insight: PortfolioInsight): void {
    this.dismissedSignal.update((current) =>
      current.filter((id) => id !== insight.insightId)
    );
  }
}
