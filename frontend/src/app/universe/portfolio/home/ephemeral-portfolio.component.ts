...
@Component({
  ...
  template: `
    <div class="wrap">
      <header class="head">
        <p class="crumb">
          <a routerLink="/portfolio" i18n="@@universe.portfolio.ephemeral.back"
            >Portfolio Intelligence</a
          >
          <span> · </span>
          <span i18n="@@universe.portfolio.ephemeral.mode"
            >Ephemeral view - nothing is saved</span
          >
        </p>
        <h1 class="address" i18n="@@universe.portfolio.ephemeral.address-title">
          Address portfolio
        </h1>
        <p class="mono">{{ truncated() }}</p>
      </header>
      ...
      @if (failure(); as failure) {
        ...
        <a
          ...
          i18n="@@universe.portfolio.ephemeral.back-home"
          >Back to Portfolio Intelligence</a
        >
      } @else if (summary(); as summary) {
        <section class="hero">
          <div>
            <p class="label" i18n="@@universe.portfolio.ephemeral.value">
              Priced value
            </p>
            <p class="value">
              @if (session.valuesHidden()) {
                ...
                {{ formatExact(summary.valuation.pricedValue, 'en') }}
                <span class="quote">{{ summary.valuation.quoteCurrency }}</span>
              }
            </p>
          </div>
        ...
        <section aria-label="Holdings">
          ...
          @if (holdings(); as holdingsPage) {
            <table>
              <thead>
                <tr>
                  <th scope="col" i18n="@@universe.portfolio.ephemeral.asset">
                    Asset
                  </th>
                  <th
                    scope="col"
                    i18n="@@universe.portfolio.ephemeral.quantity"
                  >
                    Quantity
                  </th>
                  <th scope="col" i18n="@@universe.portfolio.ephemeral.value">
                    Value
                  </th>
                  <th scope="col" i18n="@@universe.portfolio.ephemeral.state">
                    State
                  </th>
                </tr>
              </thead>
              <tbody>
                @for (
                  entry of holdingsPage.holdings;
                  track entry.holding.assetKey
                ) {
                  <tr>
                    <td>
                      {{ entry.holding.displayName ?? entry.holding.assetKey }}
                    </td>
                    <td>
                      {{
                        session.valuesHidden()
                          ? masked()
                          : quantityLabel(
                              entry.holding.quantityAtomic,
                              entry.holding.decimals
                            )
                      }}
                    </td>
                    <td>
                      {{
                        session.valuesHidden()
                          ? masked()
                          : entry.holding.value !== undefined
                            ? formatExact(entry.holding.value, 'en')
                            : '-'
                      }}
                    </td>
                    <td>
                      <app-portfolio-data-state
                        [state]="entry.holding.sourceState"
                      />
                    </td>
                  </tr>
                }
        ...
        <section aria-label="Recent activity">
          <h2 i18n="@@universe.portfolio.ephemeral.activity">
            Recent activity
          </h2>
          @if (activity(); as activity) {
            <ul class="events">
              @for (
                event of activity.events.slice(0, 10);
                track event.eventId
              ) {
                <li>
          ...
          <p class="soft" i18n="@@universe.portfolio.ephemeral.save-hint">
            Want this address tracked with labels, history, and a vault?
            <a routerLink="/portfolio/new">Create a portfolio</a>.
          </p>
        </section>
      } @else {
        <p role="status" i18n="@@universe.portfolio.ephemeral.loading">
...
export class EphemeralPortfolioComponent implements OnInit {
  ...
  private readonly summarySignal = signal<PortfolioV2SummaryResponse | null>(
    null
  );
  private readonly holdingsSignal = signal<PortfolioV2HoldingsPage | null>(
    null
  );
  private readonly activitySignal =
    signal<PortfolioSemanticActivityPage | null>(null);
  ...
  private readonly chain = computed(
    () => this.route.snapshot.paramMap.get('chain') ?? ''
  );
  private readonly network = computed(
    () => this.route.snapshot.paramMap.get('network') ?? ''
  );
  ...
  private async load(): Promise<void> {
    try {
      const [summary, holdings, activity] = await Promise.all([
        firstValueFrom(
          this.api.getSummary$(this.chain(), this.network(), this.address())
        ),
        firstValueFrom(
        ...
        firstValueFrom(
          this.api.getActivity$(this.chain(), this.network(), this.address())
        ),
      ]);
  ...
  protected describe(
    event: PortfolioSemanticActivityPage['events'][number]
  ): string {
