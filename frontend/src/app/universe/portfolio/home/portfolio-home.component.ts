...
export const PORTFOLIO_START_ROUTE = '/portfolio/new';
  ...
  template: `
    <div class="home">
      @switch (store.vaultKind()) {
        @case ('absent') {
          <section class="panel">
            <h1 i18n="@@universe.portfolio.home.welcome">
              Portfolio Intelligence
            </h1>
            <p i18n="@@universe.portfolio.home.intro">
              Track Bitcoin-native assets and UTXOs with exact values, honest
              coverage, and a vault that never leaves this browser.
            </p>
            <div class="actions">
              <a
                class="primary"
                [routerLink]="portfolioStartRoute"
                i18n="@@universe.portfolio.home.get-started"
                >Create a portfolio</a
              >
              <a
                [routerLink]="portfolioStartRoute"
                i18n="@@universe.portfolio.home.lookup"
                >Choose one public address</a
              >
            </div>
          </section>
        }
        @case ('locked') {
          <section class="panel locked" aria-live="polite">
            <h1 i18n="@@universe.portfolio.home.locked-title">
              Portfolio locked
            </h1>
            <p i18n="@@universe.portfolio.home.locked-copy">
              Your portfolios are protected. Nothing - names, balances, counts -
              is shown until you unlock.
            </p>
            <form (submit)="unlock($event, passphraseInput.value)">
              <label>
                <span i18n="@@universe.portfolio.home.passphrase"
                  >Passphrase</span
                >
                <input
                  #passphraseInput
              ...
              <button
                type="submit"
                class="primary"
                i18n="@@universe.portfolio.home.unlock"
              >
                Unlock
              </button>
            </form>
        ...
        @default {
          <p role="status" i18n="@@universe.portfolio.home.opening">
...
export class PortfolioHomeComponent implements OnInit {
  ...
  protected readonly portfolioStartRoute = PORTFOLIO_START_ROUTE;
  ...
  protected unlock(event: Event, passphrase: string): void {
    ...
    void this.store.unlock(passphrase).then((ok) => {
      if (ok) {
        ...
        this.unlockErrorSignal.set(
          $localize`:@@universe.portfolio.home.unlock-failed:That passphrase did not unlock the portfolio.`
        );
      }
    });
  }
  ...
  private openActive(): void {
    ...
    const returnMatch = returnUrl?.match(
      /^\/portfolio\/p\/([^/?#]+)(?:\/(?:overview|holdings|activity|performance|time-machine|utxos|insights|sources|reports))?$/
    );
    const requestedPortfolio =
      returnMatch === undefined || returnMatch === null
        ? undefined
        : this.store
            .portfolios()
            .find((portfolio) => portfolio.id === returnMatch[1]);
    if (returnUrl !== null && requestedPortfolio !== undefined) {
      void this.store
        .setActivePortfolio(requestedPortfolio.id)
        .then(() => this.router.navigateByUrl(returnUrl));
      return;
    }
    if (returnMatch !== undefined && returnMatch !== null) {
      void this.router.navigate(['/portfolio/manage']);
      return;
    }
