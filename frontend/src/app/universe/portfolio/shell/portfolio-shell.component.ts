    ...
    <div class="shell" [class.privacy]="session.valuesHidden()">
      <header class="shell-header">
        <div class="identity">
          ...
          @if (selectorOpen()) {
            <ul
              class="selector-menu"
              role="listbox"
              (focusout)="selectorOpen.set(false)"
            >
              @for (portfolio of store.livePortfolios(); track portfolio.id) {
                <li
                  role="option"
                  [attr.aria-selected]="
                    portfolio.id === store.activePortfolioId()
                  "
                >
                  <a
                    [routerLink]="['/portfolio/p', portfolio.id, 'overview']"
              ...
              <li>
                <a
                  routerLink="/portfolio/manage"
                  i18n="@@universe.portfolio.shell.manage"
                  >Manage portfolios…</a
                >
              </li>
              <li>
                <a
                  ...
                  i18n="@@universe.portfolio.shell.new"
                  >New portfolio…</a
                >
              </li>
            </ul>
        ...
        <nav
          ...
          i18n-aria-label="@@universe.portfolio.shell.sections-label"
          tabindex="0"
        >
          @for (section of sections; track section.path) {
            <a
              ...
              [attr.aria-current]="
                session.activeSection() === section.path ? 'page' : null
              "
            >
              {{ section.label }}
            </a>
        ...
        <div class="controls">
          <span class="state-chip">
            @if (store.activePortfolio(); as portfolio) {
              <app-portfolio-data-state
                [state]="data().aggregation?.state ?? 'pending'"
              />
            }
          </span>
          <button
          ...
          >
            {{ session.valuesHidden() ? 'Values hidden' : 'Hide values' }}
          </button>
          <button
            type="button"
            class="control"
          ...
          <button
            type="button"
            class="control"
            (click)="session.lockNow()"
            i18n="@@universe.portfolio.shell.lock"
          >
            Lock
          </button>
        </div>
      ...
      @if (data().loading && data().aggregation) {
        <div
          class="refresh-strip"
          role="status"
          i18n="@@universe.portfolio.shell.refreshing-hint"
        >
      ...
      @if (failedAccountCount() > 0) {
        <section class="load-failure" role="alert">
          <div>
            <strong>
              Portfolio data is unavailable for
              {{ failedAccountCount() }}
              {{ failedAccountCount() === 1 ? 'account' : 'accounts' }}.
            </strong>
            <p>
              @if (successfulAccountCount() === 0) {
                {{ firstFailureMessage() }} No portfolio total is available. The
                empty response is not a zero balance.
              } @else {
                The values shown are a partial subtotal and exclude unavailable
                accounts.
              }
            </p>
          </div>
          @if (retryableFailureCount() > 0) {
            ...
            <a class="control" routerLink="/portfolio/new">
  ...
  styles: [
    `
      ...
      .shell-header {
        ...
        flex-wrap: wrap;
        padding: 10px 4px;
        border-bottom: 1px solid var(--u-separator, rgba(0, 0, 0, 0.08));
        position: relative;
      }
      .identity {
        position: relative;
      ...
      .selector:hover {
        background: var(--u-surface-raised, rgba(0, 0, 0, 0.04));
      }
      .accent {
      ...
      .selector-menu a:hover {
        background: var(--u-surface-raised, rgba(0, 0, 0, 0.05));
      }
      .sections {
        display: flex;
        gap: 2px;
      ...
      .control[aria-pressed='true'] {
        border-color: var(--u-brand, var(--u-primary, inherit));
        color: var(--u-brand, var(--u-primary, inherit));
      }
      .refresh-strip {
        font-size: 12px;
      ...
      .privacy .control[aria-pressed='true'] {
        background: rgba(196, 0, 89, 0.08);
      }
...
export class PortfolioShellComponent {
  ...
  readonly failedAccountCount = computed(
    () =>
      new Set(
        this.data()
  ...
  readonly successfulAccountCount = computed(
    () =>
      this.data().accounts.filter((account) => account.state === 'ok').length
  );
  readonly retryableFailureCount = computed(
    () =>
      this.data().accounts.filter(
        (account) => account.state === 'failed' && account.retryable !== false
      ).length
  );
  readonly firstFailureMessage = computed(
    () =>
      this.data().accounts.find((account) => account.state === 'failed')
        ?.errorMessage ?? 'The account could not be read.'
  );
  ...
  readonly sections = [
    {
      path: 'overview',
      label: $localize`:@@universe.portfolio.section.overview:Overview`,
    },
    {
      path: 'holdings',
      label: $localize`:@@universe.portfolio.section.holdings:Holdings`,
    },
    {
      path: 'activity',
      label: $localize`:@@universe.portfolio.section.activity:Activity`,
    },
    {
      path: 'performance',
      label: $localize`:@@universe.portfolio.section.performance:Performance`,
    },
    {
      path: 'utxos',
      label: $localize`:@@universe.portfolio.section.utxos:UTXOs`,
    },
    {
      path: 'insights',
      label: $localize`:@@universe.portfolio.section.insights:Insights`,
    },
  ];
    ...
    this.router.events
      .pipe(
        ...
        map(() => {
          ...
          return index >= 0 && segments.length > index + 2
            ? segments[index + 2]
            : 'overview';
        })
      )
