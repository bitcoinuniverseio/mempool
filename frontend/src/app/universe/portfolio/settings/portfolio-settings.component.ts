    ...
    <div class="settings">
      ...
      <section class="panel">
        ...
        @if (store.vaultKind() === 'unlocked') {
          <details>
            <summary i18n="@@universe.portfolio.settings.change-passphrase">
              Change passphrase
            </summary>
            <label
              ><span i18n="@@universe.portfolio.settings.new-passphrase"
                >New passphrase</span
              ><input #newPass type="password" autocomplete="new-password"
            /></label>
            <button
              type="button"
              (click)="changePassphrase(newPass.value)"
              i18n="@@universe.portfolio.settings.change"
            >
              Change
            </button>
          </details>
          <div class="row">
            <button
              type="button"
              (click)="exportBackup()"
              i18n="@@universe.portfolio.settings.export"
            >
              Export encrypted backup (.universe-portfolio)
            </button>
            @if (downloadUrl(); as url) {
              <a
                [href]="url"
                [download]="downloadName()"
                i18n="@@universe.portfolio.settings.download"
                >Download backup</a
              >
            }
          </div>
          <div class="row">
            <label>
              <span i18n="@@universe.portfolio.settings.import-file"
                >Import encrypted backup</span
              >
              <input
                type="file"
                accept=".universe-portfolio,.json"
                (change)="importFile($event)"
              />
            </label>
            <label
              ><span i18n="@@universe.portfolio.settings.import-passphrase"
                >Backup passphrase</span
              ><input #importPass type="password"
            /></label>
            <button
              type="button"
              (click)="importBackup(importPass.value)"
              i18n="@@universe.portfolio.settings.import"
            >
              Validate and import
            </button>
          </div>
          <details>
            <summary
              class="danger"
              i18n="@@universe.portfolio.settings.delete-all"
            >
              Delete everything stored locally…
            </summary>
            <p class="soft" i18n="@@universe.portfolio.settings.delete-copy">
              Removes the vault and every local portfolio from this browser
              profile. Blockchain assets are never touched. This cannot be
            ...
            <button
              ...
              class="danger"
              (click)="wipe()"
              i18n="@@universe.portfolio.settings.delete-confirm"
            >
              Delete all local portfolio data
            </button>
          </details>
        } @else {
          <p class="soft" i18n="@@universe.portfolio.settings.locked">
...
export class PortfolioSettingsComponent {
  ...
  protected async changePassphrase(value: string): Promise<void> {
    if (value.length < 8) {
      this.messageSignal.set(
        $localize`:@@universe.portfolio.settings.passphrase-short:Use at least 8 characters.`
      );
      return;
    }
    await this.vault.changePassphrase(value);
    this.messageSignal.set(
      $localize`:@@universe.portfolio.settings.passphrase-changed:Passphrase changed; every record was re-encrypted.`
    );
  }
  ...
  protected async exportBackup(): Promise<void> {
    ...
    this.messageSignal.set(
      $localize`:@@universe.portfolio.settings.export-ready:Backup ready - download it and store it somewhere safe.`
    );
  }
  ...
  protected importFile(event: Event): void {
    ...
    void file.text().then((text) => {
      try {
        ...
        this.messageSignal.set(
          $localize`:@@universe.portfolio.settings.import-loaded:Backup file loaded - enter its passphrase to validate and import.`
        );
      } catch {
        this.messageSignal.set(
          $localize`:@@universe.portfolio.settings.import-bad-file:That file is not a valid backup.`
        );
      }
    });
  }
  ...
  protected async importBackup(passphrase: string): Promise<void> {
    ...
    try {
      const result = await this.vault.importEncrypted(
        this.pendingImport,
        passphrase
      );
      ...
      this.messageSignal.set(
        error instanceof Error
          ? error.message
          : 'The import failed; local data is unchanged.'
      );
    }
  }
  ...
  protected async wipe(): Promise<void> {
    ...
    await this.store.reload();
    this.messageSignal.set(
      $localize`:@@universe.portfolio.settings.deleted:All local portfolio data was deleted from this browser.`
    );
  }
}
