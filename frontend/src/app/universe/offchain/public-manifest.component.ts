...
export class PublicManifestComponent implements OnDestroy {
 text='';busy=false;error:string|null=null;report:any=null;private request?:Subscription;
 constructor(private api:OffchainApiService){}
 clear():void {this.request?.unsubscribe();this.report=null;this.error=null;this.busy=false;}
 ngOnDestroy():void {this.clear();this.text='';}
 verify():void {this.clear();let payload:any;try{payload=publicManifest(this.text);}catch{this.error='Invalid public manifest fields, signature context or secret-shaped material.';return;}this.busy=true;
 this.request=this.api.verifyManifest$(payload).subscribe({next:r=>{this.busy=false;if(typeof r?.signature_valid!=='boolean'||typeof r?.verified!=='boolean'||r.operator_authenticated!==null||typeof r.verification_scope!=='string'||r.scheme!==payload.signature_scheme){this.error='Unsupported manifest verification response.';return;}this.report=r;},error:()=>{this.busy=false;this.error='Manifest verification unavailable; no signature verdict established.';}});
 }
}
