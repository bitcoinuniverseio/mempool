...
export class ReservesApiService {
  ...
  constructor(
    ...
    this.apiBaseUrl = (this.stateService.env.GIT_COMMIT_HASH ? '' : 'http://127.0.0.1:8999') + '/api/v1/intelligence/reserves';
  }
  ...
  public getProviderById(providerId: string): Observable<ReserveProvider> {
    return this.http.get<ReserveProvider>(`${this.apiBaseUrl}/providers/${providerId}`);
  }

  public getSnapshots(providerId?: string): Observable<ReserveSnapshot[]> {
    const url = providerId ? `${this.apiBaseUrl}/snapshots?provider_id=${providerId}` : `${this.apiBaseUrl}/snapshots`;
    return this.http.get<ReserveSnapshot[]>(url);
  }

  public getSnapshotById(snapshotId: string): Observable<ReserveSnapshot> {
    return this.http.get<ReserveSnapshot>(`${this.apiBaseUrl}/snapshots/${snapshotId}`);
  }
