...
describe('viewing-key lifecycle and public request boundary',()=>{
 ...
 it('starts on the Zcash network this deployment is configured to read, mainnet by default',async()=>{
  const build=(env:unknown)=>new ZcashViewingKeyWorkspaceComponent({setTitle:vi.fn()} as any,{run:vi.fn(),publicBlocks:vi.fn()} as any,{markForCheck:vi.fn()} as any,{networkChanged$:new Subject<string>(),env} as any);
  expect(build(undefined).network).toBe('mainnet');expect(build({UNIVERSE_CHAIN_NETWORKS:{dogecoin:'testnet'}}).network).toBe('mainnet');expect(build({UNIVERSE_CHAIN_NETWORKS:{zcash:'testnet'}}).network).toBe('testnet');
  const scanner={run:vi.fn(()=>({promise:Promise.resolve({mode:'owned-blocks',last_hash:'ab'.repeat(32),next_height:12}),cancel:vi.fn()})),publicBlocks:vi.fn(async()=>({mode:'owned-blocks'}))};
  const c=new ZcashViewingKeyWorkspaceComponent({setTitle:vi.fn()} as any,scanner as any,{markForCheck:vi.fn()} as any,{networkChanged$:new Subject<string>(),env:{UNIVERSE_CHAIN_NETWORKS:{zcash:'testnet'}}} as any);c.viewingKey='local-test-key';c.startHeight=11;await c.scan();expect(scanner.publicBlocks.mock.calls[0].slice(0,3)).toEqual(['testnet',11,11]);c.ngOnDestroy();
 });
